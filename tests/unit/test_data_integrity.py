...
class DataIntegrityTests(unittest.TestCase):
    ...
    def test_phash_primary_detector_finds_exact_duplicate(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            manifest = DatasetManifest("fixture", root, (DatasetSample("a", first, 0), DatasetSample("b", second, 0)), 2)
    ...
    def test_complete_module_executes_all_four_methods(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            for index in range(20):
                ...
                samples.append(DatasetSample(f"s{index}", path, index % 2, source_id=f"src{index % 2}"))
            manifest = DatasetManifest("fixture", root, tuple(samples), 2)
            embeddings = rng.normal(size=(20, 8)).astype(np.float32)
            result = DataIntegrityModule(DataIntegrityConfig(neighbor_chunk_size=7)).analyze(manifest, embeddings)
            self.assertEqual(result.assessment.status.value, "completed")
            self.assertEqual(
                set(result.assessment.methods_executed),
                {"cleanlab_label_quality", "phash_cosine_duplicates", "isolation_forest", "source_concentration"},
            )
    ...
    def test_full_analysis_degrades_label_check_when_fold_counts_are_insufficient(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = DataIntegrityModule(DataIntegrityConfig(cosine_neighbors=2, neighbor_chunk_size=2)).analyze(manifest, embeddings)
