...
from sentinel.utils.hashing import parse_secret_key, sha256_file
...
def _engine_error(pillar: Pillar, method: str, exc: Exception) -> ModuleAssessment:
    finding = Finding(
        finding_type=FindingType.ENGINE_ERROR, pillar=pillar,
        affected_asset=AssetLocator("run", "current"), severity=Severity.HIGH,
        raw_score=None, decision_threshold="successful engine execution", confidence=0.70,
        confidence_normalizer="engine_error_policy_v1",
        human_readable_reason=f"{method} failed: {type(exc).__name__}: {exc}",
        evidence={"error_type": type(exc).__name__}, method=MethodIdentity(method, "1"),
        recommended_disposition=Disposition.REVIEW,
    )
    return ModuleAssessment(ModuleStatus.FAILED, (finding,), (), (UnavailableMethod(method, str(exc)),))
...
def _inference_transform(model_config: dict[str, Any]) -> Any:
    ...
    return transforms.Compose(
        [transforms.Resize(size, antialias=True), transforms.ToTensor(), transforms.Normalize(mean, std)]
    )
...
class SentinelPipeline:
    def run(self, config_path: Path) -> FinalizedRun:
        ...
        manifest = load_classification_manifest(
            _project_path(config["dataset_manifest"]), dataset_root, config["num_classes"],
        ...
        model = load_model(model_path, model_config["format"], model_config["architecture_id"], config["num_classes"])
        extractor = EmbeddingExtractor(
            _project_path(config["extractor_weights"]), PROJECT_ROOT / "cache" / "embeddings",
            batch_size=config["resources"]["embedding_batch_size"],
        )
        embeddings = np.stack([extractor.extract_cached(sample.image_path) for sample in manifest.samples])
        assessments: dict[str, ModuleAssessment] = {}

        if ledger:
            ledger.append("module_started", {"pillar": "F1"})
        try:
            f1 = DataIntegrityModule(DataIntegrityConfig(
                seed=config["seed"],
            ...
            source_assessments = []
        if ledger:
            ledger.append("module_completed", {"pillar": "F1", "status": assessments["F1"].status.value})
        ...
        for reference_path in sorted(reference_dir.glob("*.pt")) if reference_dir.is_dir() else []:
            reference_states.append(load_model(reference_path, "pytorch", model.architecture_id, model.num_classes).state_dict())
        try:
            f2 = ModelIntegrityModule().analyze(
                candidate_state=model.state_dict(), reference_states=reference_states,
                architecture_id=model.architecture_id, candidate_id=model.digest,
            )
            assessments["F2"] = f2.assessment
        except Exception as exc:
            assessments["F2"] = _engine_error(Pillar.F2, "model_integrity", exc)
        if ledger:
            ledger.append("module_completed", {"pillar": "F2", "status": assessments["F2"].status.value})

        selected_ids = set(config.get("inference_sample_ids", []))
        selected = [index for index, sample in enumerate(manifest.samples) if sample.sample_id in selected_ids]
        provenance_records: list[dict[str, Any]] = []
        f3_findings: list[Finding] = []
        incoming_logits = np.empty((0, model.num_classes), dtype=np.float32)
        if not selected:
            assessments["F3"] = _unavailable("inference_provenance", "inference_sample_ids is empty or contains no matching samples")
        else:
            try:
                ...
                for position, index in enumerate(selected, start=1):
                    generated = provenance.generate(
                        input_path=manifest.samples[index].image_path, model_path=model_path,
                        config=config, output=incoming_logits[position - 1], run_root=staging,
                        sequence_number=position,
                    )
                    provenance_records.append(generated.record)
                    if generated.record["status"] == "unsigned":
                        f3_findings.append(Finding(
                            finding_type=FindingType.PROVENANCE_UNSIGNED, pillar=Pillar.F3,
                            affected_asset=AssetLocator("inference_record", generated.record["record_id"]),
                            severity=Severity.HIGH, raw_score=None, decision_threshold="signed",
                            confidence=0.70, confidence_normalizer="unsigned_policy_v1",
                            human_readable_reason="Inference was released without authenticated provenance.",
                        ...
                        if ledger:
                            ledger.append("inference_signed", {"record_id": generated.record["record_id"]})
                atomic_write_json(staging / "artifacts" / "outputs" / "provenance_records.json", provenance_records)
                assessments["F3"] = ModuleAssessment(ModuleStatus.COMPLETED, tuple(f3_findings), ("inference_hmac_binding", "authenticated_audit_chain"), ())
            except Exception as exc:
                assessments["F3"] = _engine_error(Pillar.F3, "inference_provenance", exc)
        if ledger:
            ledger.append("module_completed", {"pillar": "F3", "status": assessments["F3"].status.value})
        ...
        if not reference_manifest_path.is_file() or not selected:
            assessments["F4"] = _unavailable("distribution_shift", "reference dataset or incoming inference batch is unavailable")
        else:
            try:
                reference_manifest = load_classification_manifest(reference_manifest_path, reference_manifest_path.parent, config["num_classes"])
                reference_embeddings = np.stack([extractor.extract_cached(sample.image_path) for sample in reference_manifest.samples])
                reference_logits = _logits(
                    model, reference_manifest, list(range(len(reference_manifest.samples))), model_config
                )
                f4 = DistributionShiftModule(
                    ...
                    reference_embeddings=reference_embeddings, incoming_embeddings=embeddings[selected],
                    reference_logits=reference_logits, incoming_logits=incoming_logits,
                ...
                shift_assessment = {
                    "characterization": f4.characterization, "entropy_ratio": f4.entropy_ratio,
                    "reference_entropy": f4.reference_entropy, "incoming_entropy": f4.incoming_entropy,
                    "model_dependency": f4.model_dependency, "ood_threshold": f4.ood_threshold,
                }
            except Exception as exc:
                assessments["F4"] = _engine_error(Pillar.F4, "distribution_shift", exc)
        if ledger:
            ledger.append("module_completed", {"pillar": "F4", "status": assessments["F4"].status.value})
        ...
        assets = {
            ...
            "model": {"asset_id": model.digest, "digest": model.digest, "external_path": str(model_path.resolve())},
            "references": {"model_count": len(reference_states)},
        }
        return GovernanceModule().finalize(
            stager=stager, assessments=assessments, assets=assets,
            source_assessments=source_assessments, shift_assessment=shift_assessment,
            coverage_manifest=coverage_manifest,
            validation_scope={
                "dataset": manifest.dataset_id, "architectures": [model.architecture_id],
                "sample_sizes": {"submitted": len(manifest.samples)}, "seeds": [config["seed"]],
                "status": "not_run",
            },
            model_path=model_path, model_digest=model.digest, seed=config["seed"], ledger=ledger,
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(json.dumps({"report_id": result.report_id, "run_dir": str(result.run_dir), "overall_disposition": result.overall_disposition.value}))
    return 0
