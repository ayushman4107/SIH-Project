...
from sentinel.utils.atomic_io import atomic_write_bytes, atomic_write_json
...
class AuditLedger:
    ...
    def verify(self, expected_length: int | None = None) -> VerificationResult:
        if expected_length is not None and len(self.entries) != expected_length:
            return VerificationResult(VerificationVerdict.CHAIN_BROKEN, "ledger length differs from expected length")
        previous_hash = GENESIS_HASH
        for expected_sequence, entry in enumerate(self.entries, start=1):
            if entry.get("sequence_number") != expected_sequence:
                return VerificationResult(VerificationVerdict.CHAIN_BROKEN, "sequence gap or reordering", expected_sequence)
            if entry.get("previous_entry_hash") != previous_hash:
                return VerificationResult(VerificationVerdict.CHAIN_BROKEN, "predecessor hash mismatch", expected_sequence)
            supplied_hmac = entry.get("entry_hmac")
            if not isinstance(supplied_hmac, str):
                return VerificationResult(VerificationVerdict.TAMPERED, "entry HMAC is missing", expected_sequence)
            unsigned = {key: value for key, value in entry.items() if key != "entry_hmac"}
            try:
                expected_hmac = ledger_hmac(self._key, unsigned)
            except (TypeError, ValueError) as exc:
                return VerificationResult(VerificationVerdict.TAMPERED, f"entry is not canonicalizable: {exc}", expected_sequence)
            if not secure_equal(supplied_hmac, expected_hmac):
                return VerificationResult(VerificationVerdict.TAMPERED, "entry HMAC mismatch", expected_sequence)
            previous_hash = sha256_bytes(canonical_json_bytes(entry))
        if expected_length is None:
            return VerificationResult(VerificationVerdict.VALID, "authenticated entries are valid; tail completeness is not established")
        return VerificationResult(VerificationVerdict.VALID, "authenticated entries and expected length are valid")
...
class InferenceProvenanceModule:
    ...
    def generate(
        ...
        *,
        ...
        base = {
            ...
            "output_contract": {"dtype": "float32", "byte_order": "little", "layout": "C", "shape": [int(released.size)]},
            "binding_hmac": None,
        }
        try:
            ...
            base["binding_hmac"] = binding_hmac(key, input_hash, model_hash, config_hash, output_hash)
    ...
    def verify(self, record: dict[str, Any], run_root: Path) -> VerificationResult:
        sequence = record.get("sequence_number") if isinstance(record.get("sequence_number"), int) else None
        if record.get("status") != ProvenanceStatus.SIGNED.value:
            return VerificationResult(VerificationVerdict.UNAVAILABLE, "record is unsigned", sequence)
        try:
            key = parse_secret_key(self.secret_value)
            ...
            checks = {
                ...
                "model_hash": sha256_file(model_path),
                "config_hash": sha256_bytes(canonical_json_bytes(json.loads(config_path.read_text(encoding="utf-8")))),
                "output_hash": sha256_bytes(canonical_output_bytes(np.load(output_path, allow_pickle=False))),
            }
            for field, actual in checks.items():
                if not secure_equal(str(record.get(field, "")), actual):
                    return VerificationResult(VerificationVerdict.TAMPERED, f"{field} mismatch", sequence)
            expected_binding = binding_hmac(key, checks["input_hash"], checks["model_hash"], checks["config_hash"], checks["output_hash"])
            if not secure_equal(str(record.get("binding_hmac", "")), expected_binding):
                return VerificationResult(VerificationVerdict.TAMPERED, "binding HMAC mismatch", sequence)
        except (OSError, KeyError, TypeError, ValueError) as exc:
            return VerificationResult(VerificationVerdict.UNAVAILABLE, f"verification artifact unavailable or invalid: {exc}", sequence)
        return VerificationResult(VerificationVerdict.VALID, "component hashes and binding HMAC are valid", sequence)
